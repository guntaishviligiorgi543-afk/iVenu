(() => {
  ...
  const seatState = { rows: [], legendTypes: [], byId: new Map(), selectedIds: new Set(), pendingIds: new Set(), interactionAbortController: null, interactionRoot: null, countdownId: null, isCheckingOut: false, loadRequestId: 0, eventId: null, runtimeDiagnostics: null };
  const escapeHtml = (value) => String(value ?? "").replaceAll("&", "&amp;").replaceAll("<", "&lt;").replaceAll(">", "&gt;").replaceAll('"', "&quot;").replaceAll("'", "&#039;");
  const validColor = (value) => /^#[0-9a-f]{6}$/i.test(value || "");
  const EVENT_SEAT_RPC_PAGE_SIZE = 1000;

  function canonicalTicketTypes() { return seatState.legendTypes.filter((ticket) => ticket.is_active); }
  function selectedTicketTypeId() {
    return [...document.querySelectorAll(".fillterByTktType")]
      .map((select) => select.value)
      .find(Boolean) || "";
  }
  function renderTicketTypeFilter(ticketTypes = canonicalTicketTypes(), preferredTicketTypeId = selectedTicketTypeId()) {
    const selected = ticketTypes.some((ticket) => ticket.ticket_type_id === preferredTicketTypeId)
      ? preferredTicketTypeId
      : "";
    const options = '<option value="">All Ticket Types</option>' + ticketTypes
      .map((ticket) => `<option value="${ticket.ticket_type_id}">${escapeHtml(ticket.ticket_type_name)}</option>`)
      .join("");
  ...
  function synchronizeTicketTypeFilter(ticketTypeId) {
    const selected = canonicalTicketTypes().some((ticket) => ticket.ticket_type_id === ticketTypeId)
      ? ticketTypeId
      : "";
    document.querySelectorAll(".fillterByTktType").forEach((select) => { select.value = selected; });
    refreshTicketLists();
  }
  function colorsByTicketType() {
    ...
    [...seatState.legendTypes, ...seatState.rows].forEach((item) => {
      ...
      if (item.ticket_type_id && !colors.has(item.ticket_type_id)) {
        if (!validColor(color)) console.error("Ticket type is missing a stored display color", item.ticket_type_id);
  ...
  function countRowsByTicketType(rows) {
    ...
    rows.forEach((row) => {
      ...
      const current = counts.get(ticketTypeId) || { ticketTypeId, ticketTypeName: row.ticket_type_name || row.ticketTypeName || "Unknown", count: 0 };
      current.count += 1;
      counts.set(ticketTypeId, current);
    });
    return Object.fromEntries([...counts.values()]
      .sort((left, right) => left.ticketTypeName.localeCompare(right.ticketTypeName))
      .map((entry) => [entry.ticketTypeName, { ticketTypeId: entry.ticketTypeId, count: entry.count }]));
  }
  ...
  function normalizeCanonicalEventSeat(row) {
    ...
    if (!normalized.event_seat_id || !normalized.ticket_type_id || !normalized.ticket_type_name || !Number.isFinite(normalized.price) || !normalized.status || !Number.isFinite(normalized.row_number) || !Number.isFinite(normalized.seat_number)) {
      throw new Error("get_event_seat_map returned a seat without its required canonical identity fields.");
    }
    return normalized;
  }
    ...
    for (let from = 0; ; from += EVENT_SEAT_RPC_PAGE_SIZE) {
      ...
      page.forEach((row) => {
        ...
        if (!eventSeatId || seenSeatIds.has(eventSeatId)) {
          throw new Error("get_event_seat_map pagination returned a missing or duplicate event_seat_id.");
        }
  ...
  function mapRowsToHallMap() {
    ...
    seatState.rows.forEach((row) => {
      if (!sections.has(row.section_id)) sections.set(row.section_id, { id: row.section_id, name: row.section_name, ticketType: row.ticket_type_id, rows: 0, seatsPerRow: 0 });
      const section = sections.get(row.section_id);
      section.rows = Math.max(section.rows, Number(row.row_number));
      section.seatsPerRow = Math.max(section.seatsPerRow, Number(row.seat_number));
    });
    hallMap.sections = [...sections.values()];
    selectedBand.tickets = selectedBand.tickets || {};
    canonicalTicketTypes().forEach((ticket) => {
      selectedBand.tickets[ticket.ticket_type_id] = { id: ticket.ticket_type_id, name: ticket.ticket_type_name, price: Number(ticket.price), currency: "₾" };
    });
  }

  function currentFilters() { return { ticketTypeId: document.querySelector(".tktListContainer .fillterByTktType")?.value || "" }; }
  function applyMapFilters() {
    const { ticketTypeId } = currentFilters();
    const hasFilter = Boolean(ticketTypeId);
    document.querySelectorAll(".stageMap [data-ticket-type-id]").forEach((node) => {
  ...
  async function loadOwnReservedSeats() {
    ...
    const { data, error } = await client.from("cart_items").select("event_seat_id, event_seats!cart_items_event_seat_id_fkey(status, reserved_until)").eq("user_id", session.user.id).not("event_seat_id", "is", null);
    if (error) throw error;
    return (data || []).filter((item) => item.event_seats?.status === "reserved").map((item) => {
      const row = seatState.byId.get(item.event_seat_id);
      return row ? { ...row, reserved_until: item.event_seats?.reserved_until || null } : null;
    }).filter(Boolean);
  }

  function stopCanonicalCountdown() { if (seatState.countdownId) clearInterval(seatState.countdownId); seatState.countdownId = null; }
  function syncCanonicalCountdown() {
    ...
    const expirations = basketTickets.map((item) => new Date(item.reservedUntil || 0).getTime()).filter((time) => Number.isFinite(time) && time > Date.now());
    if (!expirations.length) { timer.style.display = "none"; return; }
    ...
    const update = () => {
      ...
      if (seconds === 0) { stopCanonicalCountdown(); loadEventSeatMap().catch((error) => console.error("Unable to reconcile expired reservations", error)); }
    };
    update();
  ...
  function replaceBasketWithReservations(rows) {
    ...
    rows.forEach((row) => {
      ...
      basketTickets.push({ eventSeatId: row.event_seat_id, section: row.section_id, sectionName: row.section_name, row: Number(row.row_number), seat: Number(row.seat_number), type: row.ticket_type_name, price: Number(row.price), currency: "₾", serviceFee: Math.round(Number(row.price) * 0.05), ticketTypeId: row.ticket_type_id, reservedUntil: row.reserved_until });
    });
  ...
  function reportTicketConfigurationIssues() {
    canonicalTicketTypes().forEach((ticket) => {
      if (!Number.isFinite(Number(ticket.price)) || Number(ticket.price) < 0 || !validColor(ticket.display_color) || Number(ticket.canonical_seat_count) === 0) console.error("Ticket configuration issue", ticket);
    });
  }
  ...
  async function loadEventSeatMap() {
    ...
    if (requestId !== seatState.loadRequestId || selectedBand?.id !== eventId) return;
    ...
    seatState.byId = new Map(seatState.rows.map((row) => [row.event_seat_id, row]));
    ...
    console.info("Canonical get_event_seat_map runtime counts", seatState.runtimeDiagnostics);
    mapRowsToHallMap();
    renderTicketTypeFilter(canonicalTicketTypes(), isNewEvent ? "" : selectedTicketTypeId());
  ...
  window.renderTicketLegend = function renderDatabaseTicketLegend() {
    ...
    container.innerHTML = canonicalTicketTypes().map((ticket) => {
      const price = Number.isFinite(Number(ticket.price)) && Number(ticket.price) >= 0 ? `₾${Number(ticket.price)}` : "Price unavailable";
  ...
  function canonicalSeatGroups() {
    ...
    seatState.rows.forEach((row) => { if (!groups.has(row.section_id)) groups.set(row.section_id, []); groups.get(row.section_id).push(row); });
    return [...groups.values()].sort((left, right) => Number(left[0].section_order) - Number(right[0].section_order) || String(left[0].section_code).localeCompare(String(right[0].section_code)));
  }
  ...
  window.renderHallMap = function renderCanonicalSeatMap() {
    ...
    if (isTbilisiSportsPalaceEvent() && window.tbilisiSportsPalaceSeatMap) {
      try {
        ...
        if (seatState.runtimeDiagnostics) {
          ...
          console.info("Tbilisi Sports Palace map allocation runtime counts", mapResult.diagnostics);
        }
        applyMapFilters();
        window.attachSeatClickHandlers();
        return;
      } catch (error) {
        console.error("Tbilisi Sports Palace map could not bind canonical event seats", error);
        stageMap.replaceChildren();
        const message = document.createElement("p");
        message.className = "seat-map-load-error";
        message.textContent = "The Tbilisi Sports Palace seat map could not be loaded without risking an incomplete seat binding.";
    ...
    if (isBlackSeaArenaEvent() && window.blackSeaArenaSeatMap) {
      try {
        ...
        if (seatState.runtimeDiagnostics) {
          ...
          console.info("Black Sea Arena map allocation runtime counts", seatState.runtimeDiagnostics.mapAllocation);
        }
        applyMapFilters();
        window.attachSeatClickHandlers();
        return;
      } catch (error) {
        console.error("Black Sea Arena map could not bind canonical event seats", error);
        stageMap.replaceChildren();
        const message = document.createElement("p");
        message.className = "seat-map-load-error";
        message.textContent = "The Black Sea Arena seat map could not be loaded without risking an incomplete seat binding.";
        stageMap.appendChild(message);
        return;
      }
    }
    ...
    canonicalSeatGroups().forEach((sectionRows) => {
      ...
      grid.style.setProperty("--seat-columns", String(Math.max(...sectionRows.map((row) => Number(row.seat_number)))));
      sectionRows.forEach((row) => {
        ...
        node.setAttribute("aria-label", `${row.section_name}, row ${row.row_number}, seat ${row.seat_number}, ${row.ticket_type_name}, ₾${row.price}`);
  ...
  window.attachSeatClickHandlers = function attachCanonicalSeatClickHandlers() {
    const stageMap = document.querySelector(".stageMap");
    const interactionRoot = stageMap?.querySelector("[data-seat-interaction-root]") || stageMap;
    if (!interactionRoot || seatState.interactionRoot === interactionRoot) return;
    ...
    const listenerOptions = { signal: seatState.interactionAbortController.signal };
    interactionRoot.addEventListener("click", (event) => {
      const seat = event.target.closest(".seat[data-event-seat-id]");
      if (!seat || seat.classList.contains("unavailable") || seat.classList.contains("selected")) return;
      const row = seatState.byId.get(seat.dataset.eventSeatId);
      if (!row) return;
      eventState.selectedSeat = seat;
      document.querySelector(".confirmSection").textContent = row.section_name;
    ...
    interactionRoot.addEventListener("pointerover", (event) => { const seat = event.target.closest(".seat[data-event-seat-id]"); if (seat) showSeatTooltip(seat, event); }, listenerOptions);
    interactionRoot.addEventListener("pointermove", (event) => { const seat = event.target.closest(".seat[data-event-seat-id]"); if (seat) showSeatTooltip(seat, event); }, listenerOptions);
    interactionRoot.addEventListener("pointerout", (event) => { if (event.target.closest(".seat[data-event-seat-id]")) document.querySelector("#seatMapTooltip").hidden = true; }, listenerOptions);
  };
  ...
  window.addTicketToBasket = async function reserveSeat(ticket) {
    ...
    if (!row || seatState.selectedIds.has(row.event_seat_id) || seatState.pendingIds.has(row.event_seat_id)) return;
    seatState.pendingIds.add(row.event_seat_id);
    const { data, error } = await client.rpc("reserve_event_seat", { p_event_seat_id: row.event_seat_id });
    seatState.pendingIds.delete(row.event_seat_id);
    if (error) { await loadEventSeatMap().catch((refreshError) => console.error("Unable to reconcile seat state", refreshError)); window.alert(error.message || "That seat was just reserved by another customer."); return; }
    seatState.selectedIds.add(row.event_seat_id);
    row.status = "reserved";
    row.reserved_until = data?.[0]?.reserved_until || null;
    basketTickets.push({ eventSeatId: row.event_seat_id, section: row.section_id, sectionName: row.section_name, row: Number(row.row_number), seat: Number(row.seat_number), type: row.ticket_type_name, price: Number(row.price), currency: "₾", serviceFee: Math.round(Number(row.price) * 0.05), ticketTypeId: row.ticket_type_id, reservedUntil: row.reserved_until });
    if (typeof persistBasketState === "function") persistBasketState();
    renderBasket(); syncCanonicalCountdown(); updateSeatNode(row); refreshTicketLists();
  };
  window.removeTicketFromBasket = async function releaseSeat(ticket) {
    const item = basketTickets.find((candidate) => candidate.eventSeatId === ticket.eventSeatId) || basketTickets.find((candidate) => candidate.section === ticket.section && candidate.row === Number(ticket.row) && candidate.seat === Number(ticket.seat));
    if (!item?.eventSeatId) return;
    const { error } = await client.rpc("release_event_seat", { p_event_seat_id: item.eventSeatId });
    if (error) { await loadEventSeatMap().catch((refreshError) => console.error("Unable to reconcile seat state", refreshError)); window.alert(error.message || "The seat could not be released."); return; }
  ...
  window.getVisibleAvailableTickets = function getDatabaseAvailableTickets() {
    return seatState.rows.filter((row) => row.status === "available" && !seatState.selectedIds.has(row.event_seat_id)).map((row) => ({ eventSeatId: row.event_seat_id, section: row.section_id, sectionName: row.section_name, row: Number(row.row_number), seat: Number(row.seat_number), type: row.ticket_type_name.toLowerCase(), displayType: row.ticket_type_name, price: Number(row.price), currency: "₾", serviceFee: Math.round(Number(row.price) * 0.05), ticketTypeId: row.ticket_type_id }));
  };
  window.renderTicketListForContainer = function renderDatabaseTicketList(container) {
    if (!container) return;
    const list = container.querySelector(".tktsInList");
    const counter = container.querySelector(".freeTkts");
    if (!list || !counter) return;
    const typeFilter = container.querySelector(".fillterByTktType")?.value || "";
    const canonicalRows = getVisibleAvailableTickets();
    const rows = canonicalRows.filter((row) => !typeFilter || row.ticketTypeId === typeFilter);
    if (seatState.runtimeDiagnostics) seatState.runtimeDiagnostics.ticketListSource = countRowsByTicketType(canonicalRows.map((row) => ({ ticket_type_id: row.ticketTypeId, ticket_type_name: row.displayType })));
    list.innerHTML = rows.map((row) => `<div class="tktCard" data-type="${escapeHtml(row.type)}" data-ticket-type-id="${row.ticketTypeId}" data-section="${escapeHtml(row.section)}"><div class="tktInfo"><div class="sectionCont"><h5>section <p class="section">${escapeHtml(row.sectionName)}</p></h5><h5>row <p class="row">${row.row}</p></h5><h5>seat <p class="seat">${row.seat}</p></h5></div><div class="tktPriceCont"><p class="tktType">${escapeHtml(row.displayType)}</p><p class="tktprice">₾${row.price}</p></div></div><button class="addToBskt" data-event-seat-id="${row.eventSeatId}"><span>add to basket</span></button></div>`).join("");
    if (seatState.runtimeDiagnostics) {
      seatState.runtimeDiagnostics.ticketListRendered = countRowsByTicketType(rows.map((row) => ({ ticket_type_id: row.ticketTypeId, ticket_type_name: row.displayType })));
      console.info("Canonical ticket-list runtime counts", { source: seatState.runtimeDiagnostics.ticketListSource, rendered: seatState.runtimeDiagnostics.ticketListRendered, filterTicketTypeId: typeFilter || null });
    }
    counter.textContent = `available tickets (${rows.length})`;
    list.querySelectorAll(".addToBskt").forEach((button) => button.addEventListener("click", () => {
      const row = seatState.byId.get(button.dataset.eventSeatId);
      if (!row) return;
      eventState.selectedSeat = document.querySelector(`.seat[data-event-seat-id="${row.event_seat_id}"]`);
      document.querySelector(".confirmSection").textContent = row.section_name;
      document.querySelector(".confirmRow").textContent = row.row_number;
      document.querySelector(".confirmSeat").textContent = row.seat_number;
      document.querySelector(".confirmPrice").textContent = `₾${row.price}`;
  ...
  window.filterTicketList = function filterDatabaseTicketList(container) { window.renderTicketListForContainer(container); applyMapFilters(); };
  ...
  document.querySelectorAll(".fillterByTktType").forEach((select) => select.addEventListener("change", (event) => {
  ...
  document.querySelectorAll(".checkout").forEach((button) => button.addEventListener("click", async () => {
    ...
    seatState.isCheckingOut = true; button.disabled = true;
    try { const { data: orderId, error } = await client.rpc("checkout_reserved_event_seats"); if (error) throw error; await loadEventSeatMap(); window.alert(`Order ${String(orderId).slice(0, 8)} confirmed.`); }
    catch (error) { console.error("Checkout failed", error); window.alert(error.message || "Checkout could not be completed."); }
    finally { seatState.isCheckingOut = false; button.disabled = false; }
  }));
  window.addEventListener("event-seat-context", () => loadEventSeatMap().catch((error) => console.error("Unable to load event seats", error)));
  if (typeof selectedBand !== "undefined" && selectedBand) loadEventSeatMap().catch((error) => console.error("Unable to load event seats", error));
})();
