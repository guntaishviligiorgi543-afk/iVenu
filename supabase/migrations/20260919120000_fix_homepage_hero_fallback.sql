-- Correct the manual-slot fallback scope and retain manual selections in Automatic mode.
...
as $$
  with config as (
    ...
    join lateral (
      select generated_slot.slot::smallint as slot, row_number() over (order by generated_slot.slot) as fallback_position
